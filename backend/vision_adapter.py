"""Deterministic CV fallback for the DryLine-upgraded strategy engine.
...
import cv2
import numpy as np
from strategy_models import VisionObservation


def heuristic_observation(image: np.ndarray, frame_quality: float) -> VisionObservation:
    """Offline-safe visual fallback.

    Derives weak wetness evidence from gloss/brightness/saturation heuristics.
    Vision confidence is intentionally capped below 0.62 — callers should
    prefer the existing VLM pipeline when it is available.
    """
