"""Tyre strategy and temporal decision engine for DryLine-upgraded.

Integrates with the existing DryLine backend via new /api/* routes in main.py.
All imports reference strategy_models and strategy_state (non-conflicting names).
"""
...
from strategy_state import StrategySessionState, Sample
...
def _alpha_beta_update(state: StrategySessionState, measured: float, timestamp: float) -> tuple[float, float]:
